...
class CoverageChecker {
  ...
  constructor() {
    // From dist/scripts/, go up 3 levels to reach project root's parent (.planning/)
  ...
  private parseChecklist(content: string): CoverageReport {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Detect category headers (## with emoji numbers)
      ...
      if (line.includes('PRIORITY 0:') || line.includes('Priority 0:') || line.includes('P0 Critical')) {
        currentPriority = 'P0';
      ...
      if (line.startsWith('| ') && !line.includes('---') && !line.includes('ID |')) {
        ...
        if (cols.length >= 4) {
          ...
          // Skip header rows
          if (id === 'ID' || id === 'Feature' || id === 'Category') continue;
      ...
      // Parse checkbox items (handles both bold **ID**: and plain ID: formats)
      if (line.includes('- [ ]') || line.includes('- [x]')) {
        ...
        if (match) {
          ...
          // Determine status from emoji (more accurate than just checked/unchecked)
          let status: TestItem['status'] = checked ? 'PASS' : 'TODO';
  ...
  private calculateStats(items: TestItem[]): CoverageReport {
    ...
    return {
      features: {
        ...
        coverage: features.length > 0 ? Math.round((features.filter(i => i.status === 'PASS').length / features.length) * 100) : 0,
      },
      testCases: {
        ...
        coverage: testCases.length > 0 ? Math.round((testCases.filter(i => i.status === 'PASS').length / testCases.length) * 100) : 0,
      },
      byPriority: {
  ...
  private printSummary(report: CoverageReport): void {
    ...
    const overallCoverage = totalItems > 0 ? Math.round((passingItems / totalItems) * 100) : 0;
